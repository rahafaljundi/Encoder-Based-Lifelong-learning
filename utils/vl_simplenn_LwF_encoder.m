...
% All rights reserved.
%
% Modified by Rahaf Aljundi & Amal Rannen Triki 
% See ADDED TYPES below
%
...
for i=1:n
    ...
    switch l.type
        ...
        case 'crossentropyloss'
            res(i+1).x = crossentropyloss(res(i).x, l.class);
        case 'euclideanloss'
            res(i+1).x = euclideanloss(res(i).x, l.class);
        case 'sigmoideuclideanloss'
            res(i+1).x = euclideanloss(sigmoid(res(i).x), sigmoid(l.class));
        case 'standerize'
            res(i+1).x= en_standarize(res(i).x,l.mu,l.std) ;
        case 'reshape'
            res(i+1).x= en_reshape(res(i).x) ;
            %---------ADDED TYPES
...
if doder
    ...
    for i=n:-1:backPropLim
        ...
        switch l.type
            ...
            case 'crossentropyloss'
                res(i).dzdx = crossentropyloss(res(i).x, l.class, res(i+1).dzdx);
            case 'euclideanloss'
                res(i).dzdx = euclideanloss(res(i).x, l.class, res(i+1).dzdx);
            case 'sigmoideuclideanloss'
                res(i).dzdx = euclideanloss(sigmoid(res(i).x), sigmoid(l.class), res(i+1).dzdx);
            case 'standerize'
                res(i).dzdx= en_standarize(res(i).x,l.mu,l.std,res(i+1).dzdx) ;
            case 'reshape'
                res(i).dzdx= en_reshape(res(i).x,res(i+1).dzdx) ;
            %---------ADDED TYPES
